...
namespace Cryptography.WinApi
{
    public class WinApiCryptWrapper : ICrypt
    {
        ...
        public bool IsCertificateValid(System.Net.WebClient client = null)
        {
            if (_certificate == null)
                throw new Exception("Не инициализирован сертификат.");
            ...
            var references = GetCrlReferences();
            ...
            foreach (var reference in references)
            {
                ...
                try
                {
                    ...
                    {
                        var bytes = client.DownloadData(reference);
                        isCertRevoked = isCertRevoked || IsCertRevoked(bytes, out crl);
